...
class RealTimeDashboard(QMainWindow):
    def __init__(self, url: str):
        ...
        self.setWindowTitle("Levitate Monitor · UI v2")
    ...
    def _sample_shape_stats(self, values: List[float]):
        """Return bias-adjusted sample skewness and excess kurtosis.

        Excess kurtosis is reported with the normal distribution centered at 0.
        Returns None when there are too few non-constant samples for the statistic
        to be meaningful.
        """
        vals = [float(x) for x in values if math.isfinite(float(x))]
        n = len(vals)
        ...
        g1 = m3 / (m2 ** 1.5)
        skew = (math.sqrt(n * (n - 1)) / float(n - 2)) * g1 if n > 2 else None

        kurt = None
        if n >= 4:
            m4 = sum(x ** 4 for x in centered) / float(n)
            g2 = (m4 / (m2 * m2)) - 3.0
            kurt = ((n - 1) / float((n - 2) * (n - 3))) * ((n + 1) * g2 + 6.0)
    ...
    def _render_return_distribution(self) -> None:
        ...
        skewness, excess_kurtosis = self._sample_shape_stats(returns)
        ...
        rows = [
            ("Sample Skewness", self._fmt_num(skewness), "Bias-adjusted third standardized moment; negative means heavier left tail"),
            ("Sample Excess Kurtosis", self._fmt_num(excess_kurtosis), "Bias-adjusted fourth standardized moment minus 3; higher means fatter tails"),
        ...
        if hasattr(self, "returns_summary_lbl"):
            self.returns_summary_lbl.setText(
                f"Samples: {len(returns):,} | Skewness: {self._fmt_num(skewness)} | "
                f"Excess Kurtosis: {self._fmt_num(excess_kurtosis)} | Mean: {self._fmt_pct(mean_ret)} | "
    ...
    def _render_backtest_metrics(self) -> None:
        ...
        rows = [
            ...
            ("Sample Skewness", self._fmt_num(skewness), "Bias-adjusted third standardized moment"),
            ("Sample Excess Kurtosis", self._fmt_num(excess_kurtosis), "Bias-adjusted fourth standardized moment minus 3"),
